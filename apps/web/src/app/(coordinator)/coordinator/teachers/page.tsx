...
export default function CoordinatorTeachersPage() {
  ...
  const withScores = teachers?.filter((t) => t.avgScore !== null).length ?? 0;
  const atRisk = teachers?.filter((t) => t.avgScore !== null && t.avgScore < 50).length ?? 0;
  const avgGlobal =
    withScores > 0
      ? (
          teachers!.filter((t) => t.avgScore !== null).reduce((a, t) => a + t.avgScore!, 0) /
          withScores
        ).toFixed(1)
      : null;
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
          ...
          <p className="mt-1 text-2xl font-bold text-foreground">{isLoading ? "—" : avgGlobal ?? "—"}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
      ...
      <div className="rounded-xl border border-border bg-card px-5 shadow-sm">
        {isLoading ? (
          <div className="space-y-3 animate-pulse py-4">
            {[...Array(4)].map((_, i) => <div key={i} className="h-12 rounded-lg bg-muted" />)}
          </div>
        ) : teachers?.length === 0 ? (
