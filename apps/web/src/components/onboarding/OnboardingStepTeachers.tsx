...
export function OnboardingStepTeachers({ onComplete }: { onComplete: () => void }) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      await adminApi.createUser({ fullName: fullName.trim(), email: email.trim(), password, role: "TEACHER" });
      await qc.invalidateQueries({ queryKey: ["onboarding-teachers"] });
      toast.success("Professor cadastrado!");
      onComplete();
    } catch {
      toast.error("Erro ao cadastrar professor. Verifique se o e-mail já está em uso.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm font-medium text-foreground">Cadastre o primeiro professor</p>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Nome completo</label>
          <input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="ex: João Silva" className={inp} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">E-mail</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="professor@escola.com" className={inp} />
        </div>
      </div>
      <div>
